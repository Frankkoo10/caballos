    ...
    horses.forEach((h) => {
        ...
        tr.innerHTML = `
            <td style="text-align: left;">
                ...
                <strong>${h.name}</strong> (${h.colorName})
            </td>
            <td>${h.probPercent}%</td>
...
function startRace() {
    ...
    // Calculamos el límite vertical basándonos en la altura del contenedor
    const trackHeight = document.getElementById("track").clientHeight - 100; 
    ...
    let raceInterval = setInterval(() => {
        ...
        horses.forEach((h) => {
            ...
            h.position += speed;
            // Desplazamos modificando 'bottom' en lugar de 'left'
...
function finishRace(winnerId) {
    ...
    if (totalWon > 0) {
        ...
        msgEl.innerText = `Perdiste tu apuesta de $${totalBet}. El ganador fue ${winnerHorse.name} (${winnerHorse.colorName}).`;
    }
